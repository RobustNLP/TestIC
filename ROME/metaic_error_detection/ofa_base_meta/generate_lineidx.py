...
for dname in dirname:
    for fname in os.listdir(dname):
        if '.tsv' not in fname:
            continue
        fn = fname.split('.tsv')[0]
        nfn = fn+'.lineidx'
        generate_lineidx_file(os.path.join(dname, fname), os.path.join(dname, nfn))
        

# generate_lineidx_file( 'azure_1000_bg/pred.bg_images.tsv', 'azure_1000_bg/pred.bg_images.lineidx' )
